...
module.exports = {
  ...
  validate(brainstemKey, modelId, attributes, options = {}) {
    ...
    return (dispatch) => {
      ...
      const Model = storageManager.storage(brainstemKey).model;
      const validationErrors = new Model({ id: modelId }).validate(attributes, validateOptions);

      if (postValidateAction) { dispatch(postValidateAction(validationErrors, trackKey)); }
